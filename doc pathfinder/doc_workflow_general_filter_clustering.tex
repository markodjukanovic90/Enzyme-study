...
\begin{tikzpicture}[xshift=-2.5cm, 
	...
	\node[wp] (WP2) at (90:6cm) {Automation};
	\node[wp] (WP3) at (0:6.55cm) {Filtering \& Clustering \\ (F\&C)};
	\node[wp_exp] (WP4) at (-90:7cm) { Lab testing};
	\node[wp_exp] (WP5) at (180:6.55cm) {Crystallography};
...
\end{figure}
 ...
 Clustering. Based on the best structures of each sequences generated from \texttt{AlphaFold} (50 structures, with the best score from \texttt{AlphaFold}), we have applied a $k$-means algorithm, with a maximum $k$ value satisfying $silhouette \geq 0.25$ meaning to establish a reasonable set of clusters. 
 ...
 Based on the consensus, several protein structures have been picked from each cluster (for example a percentage $p$ from each of 7 clusters, or those closest to centroid of each cluster). Rest of the structures is generated random by RFDiffussion or similar ways. 
